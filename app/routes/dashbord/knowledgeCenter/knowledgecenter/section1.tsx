...
const Tabs = ({ className, defaultValue, ...props }: React.HTMLAttributes<HTMLDivElement> & { defaultValue: string }) => {
  return (
...
const TabsTrigger = ({ className, value, ...props }: React.HTMLAttributes<HTMLButtonElement> & { value: string }) => {
  return (
    <button
      className={cn(
...
export const Section1
 = (): JSX.Element => {
      ...
      files: [
        {
          ...
          description: "هنا وصف الملف الذي سوق يظهر للمستخدم وهو مكون من شرح قصير",
          fileType: "PDF",
        },
        {
          id: 2,
  ...
  return (
    <Card className="flex flex-col w-full max-w-[1216px] gap-8 p-4 bg-white rounded-2xl m-auto">
      {/* Navigation Tabs */}
        ...
        <TabsList className="flex items-center justify-center gap-1 p-1.5 w-full bg-neutral-50 rounded-xl border border-solid border-[#e9e9eb]">
          {navigationTabs.map((tab) => (
            <TabsTrigger
              key={tab.id}
              value={tab.id}
              className={`flex h-11 items-center justify-center gap-2 px-3 py-1 flex-1 rounded-md ${
                tab.isActive
                    ...
                    <div className="flex items-start justify-end gap-3 p-5 [direction:rtl]">
                      <div className="w-10 h-10">
                        <div className="relative w-8 h-10 left-1">
                          <img className="w-8 h-10" alt="Page" src="/page.svg" />
                  ...
                  <Button className="h-[29.42px] px-[11.77px] py-0 bg-[#006173] text-white rounded-[5.88px] border-[0.74px] border-[#669ccd] flex items-center gap-[2.94px]">
                    <div className="w-[17.65px] h-[17.65px] relative">
                      <img
                        ...
                        alt="Elements"
                        src={`/elements${article.id === 1 || article.id === 4 ? "" : `-${article.id - 1}`}.png`}
                      />
                    </div>
